namespace global
{
	...
	export  function isXMLName(str:string):boolean
	{
		...
		for (var i:number = 1; i < len; i++)
		{
			...
			if (isDigit(c) || isLetter(c) || (c == 46) || (c == 45) || (c == 95) || /*(c != 58) ||*/
	...
	//[87]      CombiningChar      ::=
	export const combiningCharTable:any[] = [
		0x0300, 0x0345,
		0x0360, 0x0361,
		0x0483, 0x0486,
		0x0591, 0x05A1,
	...
	export const extenderTable:any[] = [
	...
	{
		/**/ c = (+(c));
		var table:any[] = digitTable, len:number = table.length;
	...
	{
		/**/ c = (+(c));
		var table:any[] = letterTable, len:number = table.length;
		for (var i:number = 0; i < len; i += 2)
		{
			if (c >= table[i] && c <= table[i+1])
				return true;
	...
	function isCombiningChar(c:number):boolean
	{
		/**/ c = (+(c));
		var table:any[] = combiningCharTable, len:number = table.length;
		for (var i:number = 0; i < len; i += 2)
		{
			if (c >= table[i] && c <= table[i+1])
				return true;
	...
	function isExtender(c:number):boolean
	{
		/**/ c = (+(c));
		var table:any[] = extenderTable, len:number = table.length;
		for (var i:number = 0; i < len; i += 2)
		{
			if (c >= table[i] && c <= table[i+1])
				return true;
